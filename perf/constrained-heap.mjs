...
async function main() {
  ...
  if (!options.list) {
    console.log(`Running endurance harness with --max-old-space-size=${options.heapMb}`)
  }

  await new Promise((resolvePromise, rejectPromise) => {
    const child = spawn(
      process.execPath,
      [`--max-old-space-size=${options.heapMb}`, ...enduranceArgs],
      {
        cwd: process.cwd(),
        stdio: "inherit",
      },
    ...
    child.on("exit", (code) => {
      if (code === 0) {
        ...
        rejectPromise(new Error(`Constrained-heap endurance run failed with exit code ${code}`))
      }
    })
